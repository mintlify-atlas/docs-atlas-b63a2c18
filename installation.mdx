---
title: "Install Headroom via pip, npm, or Docker"
...
Installs all compressors, the proxy server, ML compression, image compression, and the evaluation framework. Does **not** include `[vector]` (requires a C++ toolchain) or `[pytorch-mps]` (macOS-only GPU offload).
...
| `all` | Everything above (minus `vector` and `pytorch-mps`) | `pip install "headroom-ai[all]"` |
