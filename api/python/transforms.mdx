...
Run the pipeline synchronously. Returns a [`TransformResult`](/api/python/config#transformresult). The `model_limit` keyword argument is required and must be provided explicitly (there is no default — passing `None` raises `ValueError`). Common kwargs:
