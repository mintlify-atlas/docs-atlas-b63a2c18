...
|  │  Detects     │   │    ├─ CodeCompressor (AST, tree-sitter)     │  |
...
- **CodeCompressor** for source code: AST parsing via tree-sitter, syntax-valid output.
...
Client request
  ...
  ──► CacheAligner.apply()
  ──► ContentRouter.apply()  →  SmartCrusher / CodeCompressor / Kompress
...
- **Code** — passes through unchanged unless `CodeCompressor` is explicitly enabled (`pip install "headroom-ai[code]"`)
